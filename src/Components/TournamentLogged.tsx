...
export default function TournamentLogged({userSession}:{userSession:User}){

    const [tournaments, setTournaments] = useState<Array<any>>()

    async function getTournaments() {
        const {data, error} = await supabase.from('competitors').select(`tournaments(tournament_id, tournament_name, active, is_public)`).eq('competitor_email', (userSession.email || ''))
