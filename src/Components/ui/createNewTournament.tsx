...
interface createNewTournament{
    ...
    seasonName:string,
    changeSeasonName:Function
}

export default function CreateNewTournament({changeNameFunction,handlePublic,handleSubmit,tournamentName,changeSeasonName,seasonName}:createNewTournament) {
    return (
        <div className=" w-full h-full p-4 flex flex-col justify-center items-center gap-4 ">
            ...
            <form onSubmit={(e) => handleSubmit(e)} className="flex flex-col items-center gap-4 w-full h-full">
                ...
                <label htmlFor="seasonName">
                    Nome da temporada:
                    <input type="text" maxLength={50} className="bg-zinc-500 h-12 w-full text-lg text-center font-semibold cursor-text " value={seasonName} onChange={(e) => changeSeasonName(e.target.value)} name="seasonName" id="seasonName" />
                </label>
